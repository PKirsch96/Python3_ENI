...
class Plateau:
    ...
    def __init__(self):
        ...
        self._creer_navires()
        self.cases_occupees = reduce(set.union, self.navires)
        ...
        for longueur in LONGUEURS_BATEAUX:
            ...
            while True:
                ...
                if random() > 0.5:
                    ...
                    # Créer le navire
                    navire = {l + c for l, c in product(repeat(lettre, longueur), chiffres)}
                else:
                    # Le navire est vertical
                    chiffre = str(rang)
                    lettres = [chr(65 + x) for x in range(premier, premier + longueur)]
                    # Créer le navire
                    navire = {l + c for l, c in product(lettres, repeat(chiffre, longueur))}
